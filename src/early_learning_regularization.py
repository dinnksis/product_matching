...
def binary_elr_loss(
    *,
    ...
    if example_indices.shape != logits.shape:
        raise ValueError("ELR example indices must align with logits")
    if target_history.ndim != 2 or target_history.shape[1] != 2:
        raise ValueError("ELR target history must have shape [examples, 2]")

    positive = logits.float().sigmoid().clamp(epsilon, 1.0 - epsilon)
    ...
    total = supervised + regularization_strength * regularizer
