...
def require_local_validation_data() -> None:
    required = (
        builder.VALIDATION_MANIFEST,
        builder.SERIALIZATION_FREQUENCY,
    )
...
def test_checked_in_notebooks_match_generator() -> None:
    ...
    expected_notebooks = tuple(
        ...
        for profile_name in builder.load_configuration()["profiles"]
    )
    if any(not path.is_file() for path in expected_notebooks):
        pytest.skip("generated architecture notebooks are not checked in")
    configuration = builder.load_configuration()
    dataset = builder.load_validation_dataset()
    for profile_name, profile in configuration["profiles"].items():
