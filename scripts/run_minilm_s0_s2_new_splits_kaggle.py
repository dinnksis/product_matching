#!/usr/bin/env python3
"""Publish, launch, and monitor the 2xT4 S0/S2 new-splits experiment."""
...
import create_minilm_s0_s2_new_splits_notebook as builder
import create_qwen_training_notebook as shared
import run_kaggle_notebook as kaggle


ROOT = Path(__file__).resolve().parents[1]
KERNEL_SLUG = "product-matching-minilm-s0-s2-new-splits"
...
def publish_code_dataset(env_file: Path, owner: str, dry_run: bool) -> dict[str, object]:
    ...
    if status.returncode == 0:
        command = cli + [
            ...
            "--message", f"S0 S2 new splits {str(manifest['code_bundle']['sha256'])[:12]}",
        ]
    else:
...
def main() -> None:
    ...
    command = [
        ...
        "--slug", KERNEL_SLUG,
        "--title", "Product Matching MiniLM S0 S2 New Splits",
