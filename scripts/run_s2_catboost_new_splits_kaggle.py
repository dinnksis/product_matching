#!/usr/bin/env python3
"""Publish, launch, and monitor the CPU S2 + CatBoost external evaluation."""
...
import create_s2_catboost_new_splits_notebook as builder
import run_kaggle_notebook as kaggle


ROOT = Path(__file__).resolve().parents[1]
KERNEL_SLUG = "product-matching-s2-catboost-new-splits"
RAW_DATASET_REF = "dinakepecheva/e-cup-human-data"
VALIDATION_DATASET_REF = "alexproger23/product-matching-validation-splits-v1"


def publish_dataset(env_file: Path, owner: str, dry_run: bool) -> dict[str, object]:
    ...
    print(f"Prepared private Dataset: {dataset_ref}")
    if dry_run:
        return manifest
    ...
    if status.returncode == 0:
        command = cli + [
            ...
            "--message", f"S2 CatBoost new splits {str(manifest['code_bundle']['sha256'])[:12]}",
        ]
    else:
...
def monitor(env_file: Path) -> None:
    ...
    kaggle.wait_for_kernel(
        ...
        wait_timeout=kaggle.env_int("KAGGLE_WAIT_TIMEOUT_SECONDS", 7200, 60),
    )
    destination = ROOT / "artifacts/kaggle" / KERNEL_SLUG
    destination.mkdir(parents=True, exist_ok=True)
    kaggle.run_command(
        cli + ["kernels", "output", kernel_ref, "-p", str(destination), "--force", "--page-size", "200"]
    )
...
def main() -> None:
    ...
    manifest = publish_dataset(env_file, owner, args.dry_run)
