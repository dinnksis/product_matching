#!/usr/bin/env python3
"""Generate, submit, monitor, and download the embedding boosting experiments."""
...
import create_embedding_boosting_notebook as builder
import create_qwen_training_notebook as shared
import run_kaggle_notebook as kaggle


ROOT = Path(__file__).resolve().parents[1]
SLUG = "product-matching-qwen-embedding-boosting"
RAW_DATASET_SLUG = "e-cup-human-data"
...
def publish_code_dataset(env_file: Path, owner: str, dry_run: bool) -> dict[str, object]:
    ...
    print(f"Prepared small code-only Dataset: {dataset_ref}")
    if dry_run:
        return manifest
    ...
    if status.returncode == 0:
        command = cli + [
            "datasets", "version", "--path", str(builder.DEFAULT_CODE_DATASET_DIR),
            "--message", f"Code {str(manifest['code_bundle']['sha256'])[:12]}",
        ]
    else:
        command = cli + ["datasets", "create", "--path", str(builder.DEFAULT_CODE_DATASET_DIR)]
    kaggle.run_command(command)
    import push_kaggle_training_dataset as dataset_uploader
    dataset_uploader.wait_until_ready(cli, dataset_ref)
    return manifest


def monitor_existing(env_file: Path) -> None:
    kaggle.load_dotenv(env_file)
    ...
    kaggle.run_command(cli + ["kernels", "output", kernel_ref, "-p", str(destination), "--force", "--page-size", "200"])
...
def main() -> None:
    ...
    notebook = builder.build_notebook(manifest)
    builder.DEFAULT_NOTEBOOK.parent.mkdir(parents=True, exist_ok=True)
    nbf.write(notebook, builder.DEFAULT_NOTEBOOK)
    command = [
        sys.executable, str(ROOT / "scripts/run_kaggle_notebook.py"), str(builder.DEFAULT_NOTEBOOK),
        "--env-file", str(env_file), "--slug", SLUG,
        "--title", "Product Matching Qwen Embedding Boosting",
        "--dataset", f"{owner}/{RAW_DATASET_SLUG}",
