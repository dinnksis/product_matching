#!/usr/bin/env python3
"""Publish, launch, and monitor the MiniLM S2 combined augmentation experiment."""
...
import create_minilm_s2_augmentation_notebook as builder
import create_qwen_training_notebook as shared
import run_kaggle_notebook as kaggle


ROOT = Path(__file__).resolve().parents[1]
KERNEL_SLUG = "product-matching-minilm-s2-combined-augmentation"
...
def publish_code_dataset(env_file: Path, owner: str, dry_run: bool) -> dict[str, object]:
    ...
    print(f"Prepared private code-only Dataset: {dataset_ref}")
    if dry_run:
        return manifest
    ...
    if status.returncode == 0:
        command = cli + [
            ...
            "--message", f"MiniLM S2 augmentation code {str(manifest['code_bundle']['sha256'])[:12]}",
        ]
    else:
...
def main() -> None:
    ...
    command = [
        ...
        "--slug", KERNEL_SLUG,
        "--title", "Product Matching MiniLM S2 Combined Augmentation",
