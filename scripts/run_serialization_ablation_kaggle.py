#!/usr/bin/env python3
"""Publish, launch, and later monitor the MiniLM serialization ablation."""
...
import create_serialization_ablation_notebook as builder
import run_kaggle_notebook as kaggle


ROOT = Path(__file__).resolve().parents[1]
KERNEL_SLUG = "product-matching-minilm-serialization-ablation"
...
def publish_code_dataset(env_file: Path, owner: str, dry_run: bool) -> dict[str, object]:
    ...
    if status.returncode == 0:
        command = cli + [
            ...
            "--message", f"Serialization ablation code {str(manifest['code_bundle']['sha256'])[:12]}",
        ]
    else:
...
def main() -> None:
    ...
    command = [
        ...
        "--slug", KERNEL_SLUG,
        "--title", "Product Matching MiniLM Serialization Ablation",
