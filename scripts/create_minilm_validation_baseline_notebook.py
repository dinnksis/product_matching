...
def build_notebook(
    dataset: dict[str, object], training_config: dict[str, object]
) -> nbf.NotebookNode:
    ...
    cells = [
        markdown(
            f"""
            # MiniLM human-only baseline: IID + hard + OOD
        ...
        code(
            ...
            TEMP_ROOT = Path("/kaggle/temp/product_matching_minilm_validation_v1")
            PROJECT_ROOT = WORKING_ROOT / "product_matching"
            PREPARED_DIR = TEMP_ROOT / "prepared"
            TOKEN_CACHE_DIR = TEMP_ROOT / "token_cache"
            OUTPUT_DIR = WORKING_ROOT / "minilm_validation_baseline_v1"
            RUNTIME_CONFIG_PATH = WORKING_ROOT / "cross_encoder_config.json"
            TRAIN_LOG = WORKING_ROOT / "minilm_validation_baseline.log"
    ...
    notebook.metadata.update(
        {
            ...
            "product_matching_training": {
                ...
                "experiment": "minilm_validation_baseline_v1",
