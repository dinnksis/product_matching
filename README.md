...
## Результаты MiniLM hyperparameter sweep

Выполнено 14 Kaggle-запусков на `model/pretrain_minilm_5ep`. Перебирались `learning_rate`, `epochs` и `max_length`; остальные параметры были зафиксированы. Метрика — macro average precision, большее значение лучше.

| Run | `max_length` | `learning_rate` | `epochs` | IID macro AP | Hard macro AP | OOD macro AP |
|---:|---:|---:|---:|---:|---:|---:|
...
- Лучший `Hard macro AP`: run 07 — `max_length=384`, `learning_rate=2e-5`, 3 `epochs`; значение `0.396926`.
- Лучший `IID macro AP`: run 14 — `max_length=512`, `learning_rate=2e-5`, 3 `epochs`; значение `0.803345`.
- Лучший `OOD macro AP`: run 12 — `max_length=512`, `learning_rate=5e-5`, 1 `epoch`; значение `0.653021`.
- Для устойчивого общего baseline рекомендуется run 07: он лучший на `Hard`, почти не уступает run 14 на `IID` и занимает второе место на `OOD`.
